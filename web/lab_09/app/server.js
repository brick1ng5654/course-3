...
app.post('/login', (req, res) => {
  ...
  if (user === '12345' && password === 'passw0rd') {
    // Устанавливаем куки для успешного логина
    res.cookie('user', user, { path: '/', httpOnly: false });
    res.cookie('pwd', password, { path: '/', httpOnly: false });
    res.status(200).send('Login successfull...');
  } else {
    // НЕ устанавливаем куки при неудачном логине
    res.status(401).send('ERROR');
  }
});

// Счётчики на сессии: visits + lastVisit (только для API роутов)
app.use((req, res, next) => {
  const nowIso = new Date().toISOString();
  // Создаем сессию только если она еще не создана
...
app.post(`${API_PREFIX}/prefs`, (req, res) => {
  ...
  res.cookie("page.color", color, optColor);

  // заодно установим куку с языком (текущий механизм уже это делает, но пусть будет тут)
  const lang = pickLang(req);
  const optLang = { ...cookieBase, ...(age ? { maxAge: age } : { maxAge: 365*24*60*60*1000 }) };
  res.cookie("lang", lang, optLang);
